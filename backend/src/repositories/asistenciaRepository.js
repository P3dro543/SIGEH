...
const findByEmpleadoYFecha = async (id_empleado, fecha) => {
  const { rows } = await pool.query(
    'SELECT * FROM ASISTENCIA WHERE id_empleado = $1 AND fecha = $2',
    [id_empleado, fecha]
  );
  return rows[0] || null;
};
...
const registrarEntrada = async (id_empleado, fecha, hora_entrada) => {
  const { rows } = await pool.query(
    'INSERT INTO ASISTENCIA (fecha, hora_entrada, id_empleado) VALUES ($1, $2, $3) RETURNING id_asistencia',
    [fecha, hora_entrada, id_empleado]
  );
  return rows[0].id_asistencia;
};

const registrarSalida = async (id_asistencia, hora_salida) => {
  await pool.query(
    'UPDATE ASISTENCIA SET hora_salida = $1 WHERE id_asistencia = $2',
...
const findByEmpleado = async (id_empleado) => {
  const { rows } = await pool.query(
    'SELECT * FROM ASISTENCIA WHERE id_empleado = $1 ORDER BY fecha DESC',
    [id_empleado]
  );
  return rows;
};
